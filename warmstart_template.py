...
if __name__ == "__main__":
    ...
    parser.add_argument("--evaluate_test", action="store_true", help="Enable test evaluation")
    ...
    results_serializable = results.copy()  # Create a copy to avoid modifying the original 'results' dictionary
    for key, value in results_serializable['args'].items():
        if isinstance(value, Path):
            results_serializable['args'][key] = str(value)

    for item in results_serializable['items']:
        for key, value in item['config'].items():
            if isinstance(value, Path):
                item['config'][key] = str(value)

    results_path = experiment_dir / "results.json"
    logging.info(f"Writing results to {results_path}")
    with results_path.open("w") as fh:
        json.dump(results_serializable, fh, indent=4)
