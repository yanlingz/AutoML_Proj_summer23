...
def load_deep_woods(
    datadir: Path,
    data_augmentations: Union[
        nn.Module, Callable
    ],  # data transformations, such as Recrop, HorizontalFlip etc..
    balanced: bool = False,
    resize: tuple[int, int] = (16, 16),
    # TODO: Auto download and unpack
    download: bool = True,
) -> tuple[Dimensions, CustomImageFolder, CustomImageFolder]:
    ...
    pre_processing = T.Compose(
        [
            *[augment for augment in data_augmentations.values()],
            T.Resize(
                (img_width, img_height), antialias=True
            ),  # antialias was added due to a warning from PyTorch transforms version 2
            T.ToTensor(),
        ]
    )
    ...
    val_dataset = ImageFolder(
        ...
        transform=T.Compose(
            [
                T.Resize((img_width, img_height), antialias=True),
                T.ToTensor(),
            ]
        ),
    ...
    )  # no augmentations for the test set

    return dimensions, train_dataset, val_dataset, test_dataset
...
def load_fashion_mnist(
    ...
    pre_processing = T.Compose([T.ToTensor(), *transform])
