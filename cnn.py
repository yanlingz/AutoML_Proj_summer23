...
class Model(nn.Module):
    ...
    def __init__(
        ...
        dropout_rate = config.get("regularization:dropout_rate") or config.get(
            "dropout_rate"
        )  # either use the constant dropout rate from the default config, or the varying
        # dropout rate from the regulurization search space, since the latter is sampled as a second step, we can use an or statement.
    ...
    def train_fn(
        ...
        transform: Union[
            CutMix, MixUp
        ],  # cut mix or mix up are batch augmentations, and must be used for the batch and the train_fn is the best place for that
        ...
        """
        # Using cutmix or mix up creates new labels, causing conflicts for the StatTracker. I removed it from the train_fn, since
        # we were not really interested in the train error anyway during optimization
    ...
    def eval_fn(
        ...
        with torch.no_grad():  # no gradient needed
            for images, labels in t:
                ...
                # t.set_description(f"(=> Test) Score: {score_tracker.avg:.4f}") # I did not really need this
