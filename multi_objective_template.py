...
def cnn_from_cfg(cfg: Configuration, seed: int) -> dict[str, float]:
    """
    ...
    results = {"error": val_error, "cost": cost, "macs": macs / 1e6}  # in Millions
    return results
...
# i.e., budget is passed to the target algorithm if it is present in the signature
# This is specific to SMAC
def cnn_from_cfg_test(cfg: Configuration, seed: int) -> float:
    """
    Creates an instance of the torch_model and fits the given data on it.
    This is the function-call we try to optimize. Chosen values are stored in
    the configuration (cfg).
    ...
    results = {"error": test_error, "cost": cost, "macs": macs / 1e6}  # in Millions
    return results
...
if __name__ == "__main__":
    ...
    logging.info(
        "Optimization done, now calculating the default and incumbent performance."
    )
    logging.info("------ Summary ------")

    logging.info(f"Default configuration: {configspace.get_default_configuration()}")
    default_cost = smac.validate(
        configspace.get_default_configuration(), seed=args.seed
    )
    logging.info(
        f"Default validation accuracy and training time: {1-default_cost[0], default_cost[1]}\n"
    )

    logging.info(
        "Validated objectives from the Pareto front (incumbents): (accuracy, runtime, macs)"
    )
